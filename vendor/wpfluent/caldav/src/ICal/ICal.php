...
class ICal
{
    ...
    protected function addCalendarComponentWithKeyAndValue($component, $keyword, $value)
    {
        ...
        switch ($component) {
            ...
            case 'VJOURNAL':
                $this->cal[$component][$this->journalCount - 1][$keyword] = $value;
        ...
        if ($events !== array()) {
            foreach ($events as $key => $anEvent) {
                foreach (array('DTSTART', 'DTEND', 'RECURRENCE-ID') as $type) {
                    if (isset($anEvent[$type])) {
                        ...
                        if (isset($anEvent["{$type}_array"][0]['TZID'])) {
                            $timeZone = $this->escapeParamText(
                                $anEvent["{$type}_array"][0]['TZID']
                            );
                ...
                if (isset($anEvent['RECURRENCE-ID'])) {
                    ...
                    $recurrenceDateUtc = $this->iCalDateToUnixTimestamp(
                        $anEvent['RECURRENCE-ID_array'][3]
                    );
