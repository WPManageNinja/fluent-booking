...
class URL
{
	...
	public function verify($url)
	{
		...
		$expiresAt = Arr::get($result, 'expiresAt');

		$urlSignature = Arr::get($result, 'signature');
		...
		if (!($isValidSignature = hash_equals($urlSignature, $newSignature))) {
			throw new InvalidSignatureOrExpired('Invalid Signature.');
		}
		...
		if ($expiresAt) {
			...
			if ($isExpired = $now > $expiresAt) {
				throw new InvalidSignatureOrExpired('URL Expired.');
			}
		}
	...
	public function isValidSignedUrl($url)
	{
		...
		} catch (InvalidSignatureOrExpired $e) {
			return false;
		}
	}
	...
	protected function getSignature($digest)
	{
		return hash_hmac(
        	'sha256',
        	$digest,
        	App::make('config')->get('app.key', 'secret')
        );
	}
