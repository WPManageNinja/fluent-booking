<div class="fcal_input_wrap fcal_input_multi_guests_wrap">
    {#each form[field.name] as guest, index}
        <div class="fcal_multi_guest_input">
            <input name={form[field.name]} type="email" placeholder="Email" bind:value={guest}>
    ...
    {/each}
    <button type="button" class="fcal_add_guest_btn" on:click={(() => handleAddGuest())}>+ Add Another</button>
</div>
<script>
