<script>
    ...
    const start_time = selectedDate.start;

    const bookings = JSON.stringify({timezone, start_time});

    let dispatch = createEventDispatcher();

    function resetSelection() {
        const elem = document.getElementById(`fcal_error_${appData.form_id}`);
        elem.textContent = '';
        
        const parentElem = elem.previousElementSibling;
        const childElem = parentElem.firstElementChild;
        childElem.style.borderColor = '';
...
<div>
    <input type="hidden" name={appData.name} value={bookings} />
</div>