...
class ReportController extends Controller
{
    ...
    private function getBookingWidgetStats($startTime, $endTime)
    {
        ...
        $currentMonthStats = $this->getBookingWidgetNumbers($startTime, $endTime);
        $lastMonthStats    = $this->getBookingWidgetNumbers($lastMonthStartTime, $startTime);

        $bookingStats['bookedStat']    = $this->getPercentage($currentMonthStats['totalBooked'], $lastMonthStats['totalBooked']);
        $bookingStats['completedStat'] = $this->getPercentage($currentMonthStats['totalGuests'], $lastMonthStats['totalGuests']);
        $bookingStats['cancelledStat'] = $this->getPercentage($currentMonthStats['bookingCompleted'], $lastMonthStats['bookingCompleted']);
        $bookingStats['guestStat']     = $this->getPercentage($currentMonthStats['bookingCancelled'], $lastMonthStats['bookingCancelled']);
    ...
    {
        $statusQuery = Booking::whereBetween('start_time', [$startTime, $endTime]);
        $bookedQuery = Booking::whereBetween('created_at', [$startTime, $endTime]);

        $totalBooked = $bookedQuery->count();
        $totalGuests = $bookedQuery->distinct()->count('email');

        $clonedQuery      = clone $statusQuery;
        $bookingCompleted = $statusQuery->where('status', 'completed')->count();
        $bookingCancelled = $clonedQuery->where('status', 'cancelled')->count();
    ...
    private function getAllBookingWidgetNumbers()
    {
        ...
        $totalBooked = $bookingQuery->count();
        $totalGuests = $bookingQuery->distinct()->count('email');

        $clonedQuery      = clone $bookingQuery;
        $bookingCompleted = $bookingQuery->where('status', 'completed')->count();
        $bookingCancelled = $clonedQuery->where('status', 'cancelled')->count();
    ...
    private function getPercentage($currentMonthValue, $lastMonthValue)
    {
        if ($lastMonthValue && $currentMonthValue) {
            $percentageChange = round((($currentMonthValue - $lastMonthValue) / abs($lastMonthValue)) * 100);
            return $percentageChange;
        }
    
        if ($currentMonthValue) {
            return 100;
        } elseif ($lastMonthValue) {
            return -100;
        }
    ...
    private function getAccessFilteredBookingQuery()
    {
    ...
    private function getQueryByDateRange($bookingQuery, $startTime, $endTime)
    {
        $statusQuery = clone $bookingQuery;
        $bookedQuery = $bookingQuery->whereBetween('created_at', [$startTime, $endTime]);
        $statusQuery = $statusQuery->whereBetween('start_time', [$startTime, $endTime]);

        return [
            'statusQuery'  => $statusQuery,
            'bookedQuery' => $bookedQuery
        ];
    }
